...
public class DateUtil {
    ...
    public static LocalDateTime toMax(LocalTime v) {
        var time=v == null ? MAX_LOCALTIME : v;
        return toMax(LocalDateTime.of(LocalDate.now(), time));
    }

    public static LocalDateTime toMax(LocalDateTime v) {
        if (v == null) return LocalDateTime.of(LocalDate.now(), MAX_LOCALTIME);
        return LocalDateTime.of(v.toLocalDate(), MAX_LOCALTIME);
    }
