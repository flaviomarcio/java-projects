...
public class ObjectUtilTest {
    ...
    private static final String objectSrcString = getObjectString();
    //estatico para garantir que o md5 está sendo gerado como esperado
    private static final UUID objectMd5StaticUUID = HashUtil.toMd5Uuid(ObjectUtilTest.objectSrc);
    ...
    private static String getObjectString() {
        if (ObjectUtilTest.objectSrc != null){
            var objectMapper = MapperConfig.newObjectMapper();
            try {
                return objectMapper.writeValueAsString(ObjectUtilTest.objectSrc);
    ...
    public void UT_000_CREATE_FROM() {
        ...
        Assertions.assertNotNull(ObjectUtil.createFromString(ObjectCheck.class, objectSrcString));
    }
